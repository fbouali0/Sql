...
INSERT INTO QUERY (QUERYID, APP, CLAUSENAME, OWNER, DESCRIPTION, CLAUSE, ISPUBLIC, LANGCODE, INTOBJECTNAME, PRIORITY, ISUSERLIST, NOTES)
VALUES (QUERYSEQ.NEXTVAL, 'N_METERS_SHIPMENTRECEIPTS', 'MY RDC meters shipment from CPM', 'MAXADMIN', 'Shipment of Meters from CPM to my RDC', 'exists(select 1 from invuse where invuse.invusenum = shipment.invusenum and invuse.siteid = shipment.siteid and invuse.receipts != ''COMPLETE'' and INVUSE.STATUS != ''CANCELLED''   AND invuse.FROMSTORELOC IN (SELECT location FROM LOCATIONS WHERE n_TYPE = ''CPM''))
  and exists (
    select 1 
    from shipmentline 
    where shipmentline.shipmentnum = shipment.shipmentnum 
...
INSERT INTO QUERY (QUERYID, APP, CLAUSENAME, OWNER, DESCRIPTION, CLAUSE, ISPUBLIC, LANGCODE, INTOBJECTNAME, PRIORITY, ISUSERLIST, NOTES)
VALUES (QUERYSEQ.NEXTVAL, 'N_METERS_SHIPMENTRECEIPTS', 'MY RDC meters shipment from LAB', 'MAXADMIN', 'Shipment of  Meters from LAB to my RDC', 'exists(select 1 from invuse where invuse.invusenum = shipment.invusenum and invuse.siteid = shipment.siteid and invuse.receipts != ''COMPLETE'' and INVUSE.STATUS != ''CANCELLED''  AND invuse.FROMSTORELOC IN (SELECT location FROM LOCATIONS WHERE n_TYPE = ''LAB''))
  and exists (
    select 1 
    from shipmentline 
    where shipmentline.shipmentnum = shipment.shipmentnum 
...
INSERT INTO QUERY (QUERYID, APP, CLAUSENAME, OWNER, DESCRIPTION, CLAUSE, ISPUBLIC, LANGCODE, INTOBJECTNAME, PRIORITY, ISUSERLIST, NOTES)
VALUES (QUERYSEQ.NEXTVAL, 'N_METERS_SHIPMENTRECEIPTS', 'MY CPM meters shipment from LAB', 'MAXADMIN', 'Shipment of meters from LAB to my CPM', 'exists(select 1 from invuse where invuse.invusenum = shipment.invusenum and invuse.siteid = shipment.siteid and invuse.receipts != ''COMPLETE'' and invuse.status != ''CANCELLED''  and invuse.fromstoreloc in (select location from locations where n_type=''LAB''))
  and exists (
    select 1 
    from shipmentline 
    where shipmentline.shipmentnum = shipment.shipmentnum 
